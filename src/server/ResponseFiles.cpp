...
/*   Updated: 2024/01/16 10:11:16 by skunert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/ResponseFiles.hpp"

std::string  check_and_add_header(std::string const& type, bool is_valid, MIME_type data){
...
void  ResponseFiles::storeFileIntoMap(const std::string& name, const std::string& path, MIME_type data)
{
  bool  is_valid = true;
  ...
  if (it == this->_responseFiles.end())
  {
    ...
    fileContent = check_and_add_header(path.substr(path.find_last_of(".") + 1), is_valid, data) + fileContent;
