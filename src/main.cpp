...
/*   Updated: 2024/02/19 20:05:30 by rnauke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void clientsInit(struct pollfd* pollfds)
{
...
Server* matchRequestToServer(std::vector<Server*> servers, Clients* client)
{
	...
	for (std::vector<Server*>::iterator i = servers.begin(); i < servers.end(); ++i)
	{
		...
		{
			if ((*i)->getServername() == hostname.substr(0, hostname.find(':'))
				&& (*i)->getPort() == hostname.substr(hostname.find(':')+1))
				return *i;
		}
	}
	return (Server*)NULL;
	...
	while (true)
	{
		...
		for (size_t i = 0; i < MAX_CLIENTS; i++)
		{
			...
			else if ((pollfds[i].revents & POLLERR) || (pollfds[i].revents & POLLNVAL))
			{
				...
				std::cout << pollfds[i].events << std::endl;
...
int main(int argc, char** argv)
{
  ...
  try
  {
	...
	{
    	servers.push_back(new Server(data_types, statuscodes, pollfds, clientDetails, *i.base()));
	}
