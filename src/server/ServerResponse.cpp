...
/*   Updated: 2024/02/08 18:14:53 by njantsch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  Server::getMethod(size_t idx, std::string& tmp)
{
  ...
  if (!msg.empty())
  {
    ...
    std::string response = check_and_add_header(200, this->_data[requestType], length, this->_codes) + msg;
  ...
  else if (uri.find("/?searchTerm=") != std::string::npos) // searching for file in url if it's already there
  {
    ...
    if (tmp == filename)
    {
      ...
      std::string length = Server::ft_itos(msg.size());
      std::string response = check_and_add_header(200, this->_data["html"], length, this->_codes) + msg;
      send(this->_clientPollfds[idx].fd, response.c_str(), response.size(), 0);
    }
    else
    {
      ...
      std::string length = Server::ft_itos(msg.size());
      std::string response = check_and_add_header(404, this->_data["html"], length, this->_codes) + msg;
      send(this->_clientPollfds[idx].fd, response.c_str(), response.size(), 0);
    }
  }
  else
  {
    ...
    std::string response = check_and_add_header(404, this->_data["html"], length, this->_codes) + errorMsg;
    send(this->_clientPollfds[idx].fd, response.c_str(), response.size(), 0);
  }
}
...
void  Server::notImplemented(size_t idx)
{
  ...
  std::string response = check_and_add_header(501, contentType, length, this->_codes) + msg;
  send(this->_clientPollfds[idx].fd, response.c_str(), response.size(), 0);
}
...
void  Server::methodNotAllowed(size_t idx)
{
  ...
  std::string length = Server::ft_itos(msg.size());
  std::string contentType = this->_data[this->_requests.getRequestType()];
  std::string response = check_and_add_header(405, contentType, length, this->_codes) + msg;
  send(this->_clientPollfds[idx].fd, response.c_str(), response.size(), 0);
}
